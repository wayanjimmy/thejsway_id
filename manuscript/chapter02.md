...
console.log(a);
```
![Mengisikan nilai ke variabel](https://github.com/bpesquet/thejsway/raw/master/manuscript/images/chapter02-02.png)

Kami memodifikasi variabel dengan mengisikan nilai `a` = 3.14 dibaca sebagai "a menerima nilai 3.14".
...
```js
const a = 3.14; // Nilai variabel a tidak dapat diubah
a = 6.28; // Tidak bisa!
```    
...
b += 1; // nilai b ditambahkan dengan 1
b++; // b bernilai 2
console.log(b); // 2
```
...
let num1 = 0;
{
    num1 = 1; // OK : karena num1 dideklarasi diatas block
    const num2 = 0;
}
console.log(num1); // OK : num1 di deklarasi di block yang sama
console.log(num2); // Error! num2 tidak dapat diakses karena deklarasinya di dalam scope yang berbeda
```
...
```js
// 3 adalah sebuah ekspresi yang menghasilkan nilai 3
const c = 3;
// c adalah ekspresi yang nilainya c (3)
let d = c;
// (d + 1) adalah ekspresi yang menambahkan nilai d dengan 1 (4)
d = d + 1; // d sekarang bernilai 4
console.log(d); // 4
```
...
console.log(`Saya tinggal di ${country}`); // Menampilkan "Saya tinggal di Indonesia"
const x = 3;
const y = 7;
...
Kadang kita perlu mengubah tipedata dari satu ke lain secara eksplisit. Javascript memiliki perintah `Number()` dan `String()` yang bisa kita gunakan.
